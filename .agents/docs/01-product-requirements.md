...
- **NEWS-003**：通知状态为 `draft`、`scheduled`、`published`、`archived`。草稿不可被学生访问；定时通知到达发布时间后由 Worker 原子发布；归档通知不进入学生列表且学生详情统一返回 404，其既有未读公告提醒不再计入学生徽标，真实管理员仍可读取留存记录。
...
- **NEWS-009**：真实管理员可以删除通知并二次确认。`draft/scheduled` 通知在同一事务取消活动定时发布任务、解除附件绑定并物理删除；`published` 通知转为 `archived` 并立即退出学生列表、详情和有效未读提醒，历史提醒、已发送邮件、附件元数据和审计继续保留；重复删除已归档通知幂等成功。两种删除模式都必须写入脱敏审计，孤立附件按既有延迟清理策略处理。
...
- **HW-003**：作业状态为 `draft`、`published`、`closed`、`archived`。草稿不可见；到达截止时间后自动关闭；管理员可以提前关闭或归档。归档作业不进入学生列表、详情或作业内优秀作业读取，真实管理员仍可读取留存记录。
...
- **HW-008**：真实管理员可以删除作业并二次确认。未发布 `draft` 作业在同一事务取消活动定时发布任务并物理删除；`published/closed` 作业转为 `archived` 并立即从学生作业列表、详情和优秀作业入口隐藏。归档删除不得删除固定受众、正式提交版本、私密评语、优秀标记、附件或审计；重复删除已归档作业幂等成功，两种模式都写入脱敏审计。
...
- **KB-003**：平台按参考提交的语义渲染段落、九级来源标题、连续无序/有序列表、引用、代码与复制、分隔线、保留颜色和 Emoji 的提示块、合并单元格表格、连续图片画廊、白板图片和安全附件，并按 ADR-041 补充渲染飞书行内与独立 LaTeX 公式；不接收或渲染飞书任意 HTML。公式解析失败必须回退为可读 LaTeX 源文，已有安全结构中的待办块可以继续兼容显示，但不得改变参考页面的排布与交互。
...
| --- | --- | --- | --- | --- |
| AUTH-001～AUTH-012 | 注册、邮箱验证、登录、管理员用户、个人 Session、个人资料注销 | `/auth/*`、`/admin/users/*` | `users`、Session/令牌、个人业务外键、共享操作者外键、`files`、`outbox_jobs`、审计 | AUTH-T01～AUTH-T23、FILE-T10、SEC-T01、SEC-T03 |
