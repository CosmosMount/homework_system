...
- **HELP-008**：真实管理员可以在详情页二次确认后删除任意系统反馈或问题答疑。删除事务必须锁定工单、把该工单仍未读的解决提醒标为已读、写入不含标题、正文、答复或学生身份的脱敏审计后物理删除工单；成功返回 204。删除后学生本人列表/详情、管理员列表/详情和登录态公开答疑立即不可见，历史已读提醒与只追加审计保留。学生、管理员学生视图和未登录用户不得调用删除接口；不存在记录返回 `404 RESOURCE_NOT_FOUND`。本操作不新增归档状态、软删除字段或数据迁移。
...
- **FILE-007**：用户上传默认禁止可执行文件和可主动执行的 HTML/SVG；每项任务的扩展名白名单只能在全局安全白名单内收窄。唯一例外是 KB-003/KB-008 定义的飞书知识库只读快照 PE 文件，该例外不得进入作业、通知或其他用户上传入口。
...
| HW-001～HW-008 | 作业列表/详情、个人版本、管理员作业/提交 | `/assignments*`、`/admin/assignments*` | `assignments`、受众配置、`assignment_audience_users`、`assignment_extensions` | HW-T01～HW-T16 |
...
| FILE-001～FILE-007 | 作业上传器、通知附件、授权下载 | `/uploads*`、`/files/*/download-url` | `files`、`upload_sessions`、`upload_parts`、正式资源附件关联 | FILE-T01～FILE-T09 |
