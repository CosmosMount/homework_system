...
- **AUTH-012**：有效用户可以在个人资料页输入当前密码并精确确认本人规范化邮箱后注销本人账号；真实管理员可以删除任意非当前操作者账号，包括近期活跃、待验证和已禁用账号，但必须提供至少 3 个非空白字符的内部原因、管理员当前密码、目标账号规范化邮箱和“近期 PostgreSQL 与 MinIO 加密备份可恢复”的显式确认。两条路径都要求有效 Session、同源、CSRF、Argon2id 重新认证、事务级账号生命周期锁和最后一名激活管理员保护。成功后必须在同一 PostgreSQL 事务物理删除账号、Session、令牌及个人作业提交、问卷回答、反馈答疑、报名/成员关系、提醒、幂等与个人文件元数据；平台共享通知、作业、赛事、问卷、知识库运行、团队赛事版本、管理员为他人形成的业务事实和只追加审计保留并把账号归属置空。目标用户为队长时先转给最早加入的其他当前成员，无成员则解散队伍；锁定队伍低于最小人数且无豁免时转为无效。个人 MinIO 对象必须随数据库事务写入可靠、幂等且不暴露对象键的清理 Outbox，Worker 先终止 multipart 再删除对象；成功/拒绝审计不得记录密码、邮箱、姓名、学号、文件名、对象键、提交/问卷/工单正文或令牌。界面必须披露操作不可撤销以及备份保留期内仍可能存在历史副本。
...
- **TEAM-009**：真实管理员可以在填写非空内部原因并二次确认后删除任意队伍。Service 必须锁定队伍并按历史引用分层处理：没有团队提交时物理删除队伍及成员关联；已有团队提交时释放全部当前成员、清空当前队长并转为 `dissolved`，保留队伍壳、成员历史、不可变版本、评语和附件。两种模式都必须从学生入口和常规管理员队伍列表立即隐藏、释放一赛一队占用并写不含成员身份、邀请码、正文、附件名或对象键的审计。
...
- **FILE-007**：用户上传默认禁止可执行文件和可主动执行的 HTML/SVG；每项任务的扩展名白名单只能在全局安全白名单内收窄。唯一例外是 KB-003/KB-008 定义的飞书知识库只读快照 PE 文件，该例外不得进入作业、赛事、通知或其他用户上传入口。

## 邮件与异步任务

- **MAIL-001**：系统通过 SMTP 发送邮箱验证、密码重置、通知提醒、提交延期提醒和管理员显式触发的问卷填写提醒；不发送注册审批邮件，私密评语默认只生成站内通知且不在邮件中包含正文。
...
| 管理用户、创建/删除通知和作业、删除反馈答疑、管理赛事与优秀标记 | 禁止 | 禁止 | 允许 |
...
| HW-001～HW-008 | 作业列表/详情、个人版本、管理员作业/提交 | `/assignments*`、`/admin/assignments*` | `assignments`、受众配置、`assignment_audience_users`、`assignment_extensions` | HW-T01～HW-T15 |
