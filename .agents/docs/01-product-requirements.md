...
- **AUTH-007**：管理员可以分页查看所有状态、角色和方向的账号，并按姓名、学校邮箱、学号、中文或英文角色/状态在全体账号范围搜索；搜索不得只过滤当前已加载页，筛选和翻页必须保留在 URL，越界页必须回到实际末页。用户资料、角色或状态写入后必须按同一服务端查询重新协调当前页，避免筛选残留或分页跳项。管理员可以禁用和恢复账号，也可以在登录后按需维护技术方向。禁用必须填写内部原因并写入审计日志；方向为空不得影响登录。历史届次字段不再提供新的维护入口。
...
- **INT-002**：问卷状态为 `draft`、`open`、`closed`、`archived`。初次开放只允许 `draft → open`，开放问卷可以关闭，已关闭问卷可以在保留原题目、选项、二维码 token、历史最新答案、累计提交次数、时间窗口和提交上限的前提下重新进入 `open`，随后仍可再次关闭；只有 `closed → archived` 可以归档，`archived` 为不可重新开启的终态。仅 `open` 且处于可选开始/结束时间窗口内的问卷允许学生读取和填写，开放后不得修改题目结构和提交次数上限。
...
- **INT-007**：真实管理员可以在问卷为 `open` 且当前时间处于填写窗口时，选择三种互斥范围之一发送填写提醒：通过既有用户搜索手动选择 1～100 名成员、选择一个激活技术组，或选择全部激活学生。`manual` 请求只带不重复的用户 UUID，服务端整体复核均为已验证 `active student`；`direction` 只带一个技术组 UUID，`all` 不带成员或技术组，两者均由服务端在发送瞬间查询权威激活学生集合，不接受浏览器展开后的成员列表。技术组不存在/停用、范围为空或任一手动成员无效时不得创建部分任务；同一事务写入 `intention_open_email` Outbox 和包含范围、可选技术组 UUID 及请求/入队/已存在数量的脱敏审计，不记录成员身份。同一问卷 revision 下同一成员只入队一次，关闭后重新开启因 revision 变化可以再次提醒；邮件只含收件人称呼、问卷标题和站内 `/intentions/{survey_id}` 链接，不含答案、补充说明、二维码 token 或管理员信息。
...
- **KB-003**：平台按参考提交的语义渲染段落、九级来源标题、连续无序/有序列表、引用、代码与复制、分隔线、保留颜色和 Emoji 的提示块、合并单元格表格、连续图片画廊、白板图片和安全附件，并按 ADR-041 补充渲染飞书行内与独立 LaTeX 公式；不接收或渲染飞书任意 HTML。正文图片点击后必须在当前页模态浮层查看，支持遮罩、关闭按钮和 Escape 退出，不得打开新页面；连续图片按容器实际宽度尽量并排，空间不足时自然换行，飞书保留的空段落或只承载媒体的结构容器不得误将同一视觉序列拆成单图，可见文字等语义内容仍须终止分组。成功本地化的普通附件、富文本内嵌文件和目录独立文件必须继续通过登录态知识库资源接口下载，不得因关闭飞书原文入口而失效。公式解析失败必须回退为可读 LaTeX 源文，已有安全结构中的待办块可以继续兼容显示。
...
- **KB-005**：学生读取永远指向最近一次 `succeeded` 快照。对齐参考同步后，目录分页/递归、任一目标 Docx 的 blocks 或标题读取失败必须使整次运行失败，不能以跳过目录子树或单篇正文的方式发布部分新快照；图片或白板单项失败跳过对应块，正文附件失败继续保留整篇飞书原文回退元数据供真实管理员普通视图排障，目录 `obj_type=file` 独立文件本地化失败则保留不可下载节点。真实学生和管理员学生视图均不得为失败资源生成飞书链接。同步失败、Worker 重启或媒体本地化失败不得覆盖已有成功快照。
...
- **KB-008**：部署方只需填写 `FEISHU_APP_ID`、`FEISHU_APP_SECRET`（或 secret file）和 `FEISHU_WIKI_URL`；Worker 或首次上线内部运维命令从受信 Wiki URL 解析整个空间或单篇文档目标，只访问固定飞书 API HTTPS 主机，外链只允许安全协议。参考仓库的可选 `FEISHU_DOCUMENT_ID`、硬编码租户域名和公开静态 token 文件名不得引入；危险、超限或下载失败的正文附件只保留整篇飞书原文回退元数据，失败的目录独立文件只保留不可下载节点，真实学生和管理员学生视图不得渲染飞书回退链接，且任何响应或界面都不得泄露令牌、对象键或飞书错误正文。
...
| INT-001～INT-007 | 学生问卷列表/填写、管理员问卷查看/草稿编辑/统计/实名名单/二维码/三范围邮件 | `/intentions*`、`/admin/intentions*` | `intention_surveys`、`intention_questions`、`intention_options`、`intention_responses`、`intention_response_options`、`outbox_jobs` | INT-T01～INT-T17 |
