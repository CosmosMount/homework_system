...
- **AUTH-007**：管理员可以分页查看所有状态、角色和方向的账号，并按姓名、学校邮箱、学号、中文或英文角色/状态在全体账号范围搜索，也可以选择一个现有技术组只查看该组人员；搜索不得只过滤当前已加载页，技术组、搜索、活跃度筛选和翻页必须保留在 URL，越界页必须回到实际末页。用户资料、角色或状态写入后必须按同一服务端查询重新协调当前页，避免筛选残留或分页跳项。管理员可以禁用和恢复账号，也可以在登录后按需维护技术方向。禁用必须填写内部原因并写入审计日志；方向为空不得影响登录。历史届次字段不再提供新的维护入口。
...
- **AUTH-009**：登录失败对账号不存在、密码错误和不可登录状态返回统一对外文案；系统必须先把用户名或完整邮箱规范化为账号邮箱，并以真实或 dummy Argon2id 校验保持统一密码处理成本。已验证 `active` 账号输入正确密码时不得被既有失败事件施加持久等待；无效登录仍按规范化邮箱和来源 IP 在 10 分钟窗口内计数、限流并记录不含密码的安全事件。注册、验证邮件重发和密码重置申请不得使用应用层持久失败窗口，但继续记录安全事件；Nginx 瞬时入口限流继续生效。
- **AUTH-010**：邮箱、学号均全局唯一；并发注册命中任一唯一约束时必须回滚并返回对应字段的 `400 VALIDATION_ERROR`，不得转为未预期 500。管理员修改邮箱后，账号必须重新完成邮箱验证才能恢复登录。
- **AUTH-011**：管理员可以仅在当前 Session 开启“学生视图”。开启后 Session 的有效角色为 `student`，可访问学生工作台、通知、作业、校内赛队伍中心和个人提交操作；所有管理员接口必须返回 403，且前端不显示管理导航。关闭学生视图恢复当前 Session 的管理员权限；真实 `users.role` 始终不变。已打开的作业管理页若因同一 Session 在其他标签切入学生视图而写入失败，只能调用现有关闭学生视图接口并由后端复核真实 `admin` 角色后重试一次；不得直接放宽管理接口。其他管理员把该账号真实角色改为 `student` 时必须撤销该用户全部 Session，本人重新登录后不能通过学生视图按钮恢复管理员权限。
- **AUTH-012**：有效用户可以在个人资料页输入当前密码并精确确认本人规范化邮箱后注销本人账号；真实管理员可以删除任意非当前操作者账号，包括近期活跃、待验证和已禁用账号，但必须提供至少 3 个非空白字符的内部原因、管理员当前密码、目标账号规范化邮箱和“近期 PostgreSQL 与 MinIO 加密备份可恢复”的显式确认。两条路径都要求有效 Session、同源、CSRF、Argon2id 重新认证、事务级账号生命周期锁和最后一名激活管理员保护。成功后必须在同一 PostgreSQL 事务物理删除账号、Session、令牌及个人作业提交、问卷回答、反馈答疑、当前队伍成员关系、提醒、幂等与个人文件元数据；平台共享通知、作业、问卷、知识库运行和 legacy 历史数据、管理员为他人形成的业务事实和只追加审计保留并把账号归属置空。目标用户为队长时先转给最早加入的其他当前成员，无成员则解散队伍。个人 MinIO 对象必须随数据库事务写入可靠、幂等且不暴露对象键的清理 Outbox，Worker 先终止 multipart 再删除对象；成功/拒绝审计不得记录密码、邮箱、姓名、学号、文件名、对象键、提交/问卷/工单正文或令牌。界面必须披露操作不可撤销以及备份保留期内仍可能存在历史副本。
- **AUTH-013**：登录页必须提供默认关闭的“记住登录状态”选项，不得把密码或可逆密码材料写入 Cookie、浏览器持久存储或数据库。未勾选时 Session/CSRF 使用浏览器会话 Cookie，服务端继续按管理员 4 小时、学生 12 小时空闲及 14 天绝对期限校验；勾选时 Cookie、空闲期与绝对期最长均为 30 天，并把高熵 Session Cookie 与登录时可信代理提供的精确来源 IP 进行会话级 HMAC 绑定，数据库不得保存精确 IP。持久会话只有在 Cookie 有效、账号仍为 `active`、未撤销/过期且来源 IP 绑定匹配时才免重复登录；IP 变化必须立即撤销，任何情况下都不得仅凭相同 IP 查询用户或创建 Session。
...
- **NEWS-003**：通知状态为 `draft`、`scheduled`、`published`、`archived`。草稿不可被学生访问；定时通知到达发布时间后由 Worker 原子发布；归档通知不进入学生列表且学生详情统一返回 404，其既有未读公告提醒不再计入学生徽标。手工归档且未删除的留存记录仍可由真实管理员读取，执行删除的归档记录不进入常规管理列表与详情。
- **NEWS-004**：管理员可以置顶已发布通知并设置置顶结束时间。学生列表先显示有效置顶项，再按发布时间倒序显示普通项。
- **NEWS-005**：通知首次发布时必须为每个目标学生建立站内通知记录。`student_notifications` 是历史命名保留的登录用户站内提醒表，接收人可以是学生或管理员；学生可以标记单条已读或全部已读，导航按提醒实际目标分别在“通知”“作业”“校内赛”和“反馈答疑”入口显示未读数量。管理员只在“反馈答疑”入口显示本人尚未读取的新工单蓝点；已读或已失效提醒不得继续计数。
...
- **NEWS-009**：真实管理员可以删除通知并二次确认。`draft/scheduled` 通知在同一事务取消活动定时发布任务、解除附件绑定并物理删除；`published` 通知转为 `archived` 并标记删除，手工归档通知首次删除时补写删除标记；两者都立即退出学生端和常规管理列表/详情。历史提醒、已发送邮件、附件元数据和审计继续保留；对已有删除标记的重复 DELETE 幂等成功。两种删除模式都必须写入脱敏审计，孤立附件按既有延迟清理策略处理。

## 培训作业

- **HW-001**：所有真实角色为 `admin` 的管理员具有相同作业管理权限，不按首个管理员、账号创建顺序或作业创建者区分；管理员可以创建作业，字段包括标题、说明、外部培训资料链接、提交要求、允许的附件扩展名、发布时间、截止时间和状态。
...
- **SUB-001**：作业提交可以包含 Markdown 文本、外部链接和零个或多个附件；至少一种内容必须存在。学生可在一次选择中加入多张图片或其他作业允许类型的附件，每个正式版本最多引用 100 个不重复文件，类型与合计大小仍按作业冻结规则和全局安全策略校验。
- **SUB-002**：每次确认提交都创建连续递增、不可变的正式版本。上传中的临时文件不构成提交版本。
- **SUB-003**：提交聚合记录必须指向最新版本，历史版本永久保留，学生和管理员都能查看版本号、提交时间和文件清单。学生个人版本详情必须返回该版本所属的学生作业详情，管理员提交审阅详情必须返回该提交所属作业的管理编辑页，不得退回全局工作台而丢失作业上下文。
...
- **COMP-001～COMP-006（已退出当前产品）**：系统不再创建、读取或展示赛事实体、报名、阶段、赛题、赛事作品或赛事评语。“校内赛”只作为独立队伍中心的导航名称；旧赛事结构和历史提交仅由数据库 legacy 兼容层保留，不提供运行时 API。
- **TEAM-001**：已验证的 active student 无需赛事或报名即可创建队伍并成为队长，或凭一次输入的邀请码加入成形队伍；邀请凭证在数据库中只保存带服务端 pepper 的 HMAC。
- **TEAM-002**：同一学生全局最多属于一支 left_at 为空的当前队伍，由数据库部分唯一索引保证；重复或并发加入返回 409 ALREADY_IN_TEAM 或 TEAM_MEMBERSHIP_CONFLICT。
- **TEAM-003**：队长可以移除其他成员、轮换邀请码和转让队长；非队长可以退出本人。队长退出或被移除前必须转让；仅剩队长一人时可以解散队伍。
- **TEAM-004**：每支队伍保存自己的 max_members，服务端默认 5 且数据库限制为 1～20；加入和管理员补录不得超过容量。产品不设置最小人数、资格、报名截止、锁队或人数豁免。
- **TEAM-005**：无队伍学生可以主动自动分配。Service 在事务内按当前人数、创建时间和 ID 稳定选择未满 forming 队伍；无候选时自动创建队伍并只在当前响应返回一次邀请码。
- **TEAM-006**：学生队伍中心提供按名称搜索和分页的公开目录，只返回队伍名称、forming 状态、当前/最大人数和 can_join，不公开成员身份或邀请码；本人完整队伍详情只返回给当前成员。
...
stateDiagram-v2
    [*] --> forming: 学生创建或自动建队
    forming --> dissolved: 单人队伍由队长解散
~~~

## 学生问卷

- **INT-001**：管理员可以创建并查看包含标题、已清洗 Markdown 说明和 1～30 道必答选择题的完整问卷；每题包含题目、1～30 个选项并独立配置为单选或多选，标题、题目和选项去除首尾空白后不得为空，同一题内选项不得重复。`draft` 问卷允许管理员按 revision 修改标题、说明、时间窗口、提交上限、填写范围和完整题目结构；`open/closed` 允许按 revision 修改标题、说明、题目文字、时间窗口、提交上限和填写范围，但题目数量及顺序、题型、选项数量/文字/顺序冻结；`archived` 只读。
- **INT-002**：问卷状态为 `draft`、`open`、`closed`、`archived`。初次开放只允许 `draft → open`，开放问卷可以关闭，已关闭问卷可以在保留题目/选项身份、二维码 token、历史最新答案和累计提交次数的前提下重新进入 `open`，随后仍可再次关闭；只有 `closed → archived` 可以归档，`archived` 为不可重新开启或编辑的终态。仅 `open`、处于可选开始/结束时间窗口内且当前属于问卷填写范围的学生允许读取和填写；开放或关闭后的合法编辑不得删除回答、重置累计次数或轮换二维码 token。
- **INT-003**：管理员可以把每人最多提交次数设置为 1～100 的正整数或不限。登录学生每次成功保存全部题目的答案和可选补充说明即消耗一次提交；同一学生对同一问卷只保留一份最新答案和累计提交次数，达到上限后服务端必须拒绝再次提交，不限次数时可在问卷状态为 `open` 且处于填写时间窗口内继续提交，关闭与重新开启不得重置累计次数。
- **INT-004**：每道单选题必须且只能提交一个有效选项，多选题必须提交一个或多个有效选项；服务端必须验证问题属于当前问卷、选项属于对应问题、全部问题均已回答且没有重复问题/选项。学生只能读取本人的最新答案和提交次数。
- **INT-005**：管理员可以查看问卷当前目标激活学生总数、提交人数、提交率及每道题各选项的人数和比例；管理员还可以查看实名提交名单，字段限于姓名、学号、学校邮箱、最新答案、累计提交次数、最后提交时间和补充说明。学生技术组变化不删除既有回答，因此实名名单和提交人数保留已发生的回答，统计分母按查询时仍属于填写范围的激活学生计算。名单与个人答案仅管理接口可返回，不向其他学生公开，也不得写入日志或审计详情。
- **INT-006**：管理员可以为未关闭问卷生成移动端填写二维码；二维码 token 使用不可猜测随机值、数据库只保存 SHA-256，重新生成后旧 token 立即失效。二维码只定位问卷，学生扫码后仍必须登录并通过当前填写范围校验，才可返回原问卷填写页。
- **INT-007**：真实管理员可以在问卷为 `open` 且当前时间处于填写窗口时，选择三种互斥范围之一发送填写提醒：通过既有用户搜索手动选择 1～100 名成员、选择 1～100 个不重复的激活技术组，或选择问卷全部目标激活学生。`manual` 请求只带不重复的用户 UUID，服务端整体复核均为已验证且属于问卷填写范围的 `active student`；`direction` 请求只带不重复且属于问卷目标组子集的技术组 UUID 列表，`all` 不带成员或技术组，两者均由服务端在发送瞬间查询问卷受众内的权威激活学生集合，不接受浏览器展开后的成员列表。多个技术组按并集解析；任一技术组不存在/停用/越过问卷受众、范围为空或任一手动成员无效时不得创建部分任务；同一事务写入 `intention_open_email` Outbox 和包含范围、可选技术组 UUID 列表及请求/入队/已存在数量的脱敏审计，不记录成员身份。同一问卷 revision 下同一成员只入队一次，关闭后重新开启因 revision 变化可以再次提醒；邮件只含收件人称呼、问卷标题和站内 `/intentions/{survey_id}` 链接，不含答案、补充说明、二维码 token 或管理员信息。
...
- **KB-002**：阅读器采用白色主题，顶部展示知识库标题、文档数和同步时间；桌面从左到右为可折叠的 280 px 文档目录、正文和正文右侧可折叠的 180 px 本文目录。右侧本文目录必须随页面滚动吸附在视口内、长目录可独立滚动，并根据阅读位置高亮当前章节。无有效当前文档时，文档目录树只展示根层条目且所有根文件夹保持收起，不得自动选择或读取第一篇文档；进入、切换或通过浏览器历史恢复有效文档时必须只自动展开当前文档的祖先链、把活动项滚入目录可视区域，并收缩离开的旧自动路径；浏览器历史返回无当前文档的首页时必须恢复根层收起视图，文档目录栏自身的整体开合状态仍由用户控制。成功打开文档后同时折叠系统最左侧主要导航，加载失败时保留当前文档、目录树和主导航状态。支持标题搜索、URL 查询参数及飞书内部文档跳转；真实学生和管理员学生视图不得显示飞书原文入口或未映射飞书文档外链，显式映射到当前快照的文档 mention 继续在阅读器内切换；真实管理员普通视图可以保留原文入口用于排障。文档目录、搜索框和目录按键必须使用系统统一控件样式。移动端继续使用右下固定目录按钮打开包含文档搜索、目录树和本文目录的全屏目录。
- **KB-003**：平台按参考提交的语义渲染段落、九级来源标题、连续无序/有序列表、引用、代码与复制、分隔线、保留颜色和 Emoji 的提示块、合并单元格表格、连续图片画廊、白板图片和安全附件，并按 ADR-041 补充渲染飞书行内与独立 LaTeX 公式；不接收或渲染飞书任意 HTML。飞书图片块的非空文字说明必须随图片保留，在网页中显示于对应图片下方，并用于图片及页内预览的可访问名称；无说明时继续使用安全文件名或通用名称。正文图片点击后必须在当前页模态浮层查看，支持遮罩、关闭按钮和 Escape 退出，不得打开新页面；连续图片按容器实际宽度尽量并排，空间不足时自然换行，飞书保留的空段落或只承载媒体的结构容器不得误将同一视觉序列拆成单图，可见文字等语义内容仍须终止分组。成功本地化的普通附件、富文本内嵌文件、受信飞书 Drive 文件 mention/链接和目录独立文件必须继续通过登录态知识库资源接口下载，不得因关闭飞书原文入口而失效；Drive 文件不得再按普通文档 mention 显示为纯文字。知识库快照允许经 PE 结构校验的 Windows `.exe` 作为只下载附件，但不得内联展示或自动运行。未本地化文件必须明确显示“文件类型不支持”“文件超过同步上限”或“暂不可下载”，不得静默变成无状态文本。公式解析失败必须回退为可读 LaTeX 源文，已有安全结构中的待办块可以继续兼容显示。
...
- **KB-005**：学生读取永远指向最近一次 `succeeded` 快照。对齐参考同步后，目录分页/递归、任一目标 Docx 的 blocks 或标题读取失败必须使整次运行失败，不能以跳过目录子树或单篇正文的方式发布部分新快照；图片或白板单项失败跳过对应块，正文附件及 Drive 文件引用失败继续保留粗粒度不可用状态和真实管理员排障回退元数据，目录 `obj_type=file` 独立文件本地化失败则保留不可下载节点。资源失败只能归一为 `type_not_allowed`、`too_large`、`unavailable`，真实学生和管理员学生视图均不得为失败资源生成飞书链接。同步失败、Worker 重启或媒体本地化失败不得覆盖已有成功快照。
- **KB-006**：知识块存 PostgreSQL JSONB；图片、白板、安全附件和目录独立文件存 MinIO，对象键只由服务端生成，文件内容不得写入 PostgreSQL；目录节点只保存可空内部资源外键及安全元数据。
- **KB-007**：知识库目录、文档和媒体接口全部要求登录；媒体下载必须确认资源被当前成功快照的文档资源关联或独立文件目录节点引用后返回短时签名地址。关闭学生端飞书原文入口不得改用未鉴权文件地址，也不得影响已本地化普通附件、富文本内嵌文件和目录独立文件的当前快照授权下载。
- **KB-008**：部署方只需填写 `FEISHU_APP_ID`、`FEISHU_APP_SECRET`（或 secret file）和 `FEISHU_WIKI_URL`；Worker 或首次上线内部运维命令从受信 Wiki URL 解析整个空间或单篇文档目标，只访问固定飞书 API HTTPS 主机，外链只允许安全协议。正文富文本只有受信飞书/Lark 租户主机上严格匹配 `/file/{token}` 且 token 由安全字符组成时，才能作为 Drive 文件引用进入 `files` 下载链路；非受信、额外路径或编码斜杠不得触发下载。参考仓库的可选 `FEISHU_DOCUMENT_ID`、硬编码租户域名和公开静态 token 文件名不得引入。合规知识库文件默认最多 1 GiB，图片和白板默认最多 50 MiB；大文件必须使用有界流式 Feishu→MinIO multipart，不得把完整响应载入 Worker 内存。仅知识库文件链路可在 64 KiB 首块通过 DOS/PE 签名、结构、架构和可执行映像校验后接受单一语义 `.exe`；伪装双扩展、DLL 和不完整/未知 PE 必须拒绝。危险、超限或下载失败的正文文件只保留稳定粗粒度状态与真实管理员排障元数据，失败的目录独立文件只保留不可下载节点，真实学生和管理员学生视图不得渲染飞书回退链接，且任何响应、界面或日志都不得泄露令牌、对象键、文件名或飞书错误正文。

## 反馈答疑

- **HELP-001**：登录学生可以创建反馈答疑工单，类型只能是“系统反馈”或“问题答疑”；每条工单必须包含去除首尾空白后非空的标题和详情。详情使用服务端统一安全 Markdown 清洗，不接收附件、评分或手动公开设置。创建事务必须同时为全部当前 `active admin` 写入幂等站内提醒，任一步失败均不得留下工单、审计或部分管理员提醒。
...
- **HELP-003**：真实管理员可以按类型、状态和关键词分页查看全部工单及提交学生的姓名、学号和学校邮箱，并读取完整详情；管理侧提供只统计当前管理员本人 `help_request_created` 未读提醒的数量接口，详情只返回当前管理员、当前工单的未读提醒 ID，供页面打开后复用受保护的单条已读接口。学生和管理员学生视图不得访问任何管理接口。
...
- **HELP-005**：管理员首次答复或修订答复时，必须在同一事务写入审计和一条幂等学生站内通知；通知只包含安全标题和本人详情链接，不包含问题、反馈或答复正文，不发送邮件。首次答复还必须使该工单面向全部管理员的未读创建提醒失效；修订答复不重新操作管理员创建提醒。事务失败不得留下半成品答复、审计或通知；未读解决提醒显示在学生“反馈答疑”入口，学生打开对应本人工单详情后通过受保护写接口标记该工单提醒为已读。
...
- **HELP-008**：真实管理员可以在详情页二次确认后删除任意系统反馈或问题答疑。删除事务必须锁定工单、把该工单全部仍未读的管理员创建提醒与学生解决提醒标为已读、写入不含标题、正文、答复或学生身份的脱敏审计后物理删除工单；成功返回 204。删除后学生本人列表/详情、管理员列表/详情和登录态公开答疑立即不可见，历史已读提醒与只追加审计保留。学生、管理员学生视图和未登录用户不得调用删除接口；不存在记录返回 `404 RESOURCE_NOT_FOUND`。本操作不新增归档状态、软删除字段或数据迁移。

## 优秀作业

- **SHOW-001**：管理员只可以把不可变的个人作业提交版本标记为优秀作业；其他来源不得标记。
...
- **FILE-001**：每个正式提交版本的全部附件合计不得超过 2 GiB；管理员可为单个作业配置更小限制，不得超过全局上限。
...
- **FILE-005**：附件下载或页内预览必须先通过完全相同的业务关联授权，再返回 5 分钟预签名地址；学生不能通过猜测 `file_id` 或对象键绕过权限。通知附件、本人作业版本、管理员提交审阅和作业内优秀作业统一提供安全类型的页内预览，并始终保留下载能力。
- **FILE-006**：上传会话 24 小时无活动后过期。未关联正式版本的已完成对象保留 24 小时后由 Worker 清理；引用中的对象不得自动清理。
- **FILE-007**：用户上传默认禁止可执行文件和可主动执行的 HTML/SVG；每项任务的扩展名白名单只能在全局安全白名单内收窄。页内预览只接受服务端检测结果与扩展名一致的 PDF、PNG/JPEG/GIF/WebP、MP4/WebM，以及服务端确认不含 NUL 的安全纯文本/源码；不得用客户端声明媒体类型兜底。HTML、SVG、Office、压缩包、可执行文件、未知或不匹配类型只允许下载，预览请求返回 `415 FILE_PREVIEW_NOT_SUPPORTED`。唯一上传例外是 KB-003/KB-008 定义的飞书知识库只读快照 PE 文件，该例外不得进入作业、通知或其他用户上传入口。

## 邮件与异步任务

- **MAIL-001**：系统通过 SMTP 发送邮箱验证、密码重置、通知提醒、提交延期提醒、作业私密评语更新提醒和管理员显式触发的问卷填写提醒；不发送注册审批邮件。评语邮件只含称呼、作业标题和本人站内提交链接，不包含评语正文。
...
| 查看全部反馈答疑并填写/修订答复或删除工单 | 禁止 | 禁止 | 仅真实管理员视图 |
| 注销本人账号 | 禁止 | 重新认证后仅本人 | 重新认证后仅本人，仍保护最后激活管理员 |
| 管理员永久删除其他账号 | 禁止 | 禁止 | 真实管理员重新认证、精确确认并确认备份后允许 |
| 管理用户、创建/删除通知和作业、删除反馈答疑、管理队伍与优秀标记 | 禁止 | 禁止 | 允许 |
...
| FILE-001～FILE-007 | 作业上传器、通知附件、授权下载与安全页内预览 | `/uploads*`、`/files/*/download-url`、`/files/*/preview-url` | `files`、`upload_sessions`、`upload_parts`、正式资源附件关联 | FILE-T01～FILE-T12 |
| MAIL-001～MAIL-005 | 认证邮件提示、通知/问卷邮件结果、管理员邮件任务 | 认证邮件触发接口、通知发布、问卷范围邮件、`/admin/mail-outbox*` | `outbox_jobs`、`student_notifications`、唯一事件键 | MAIL-T01～MAIL-T04、NEWS-T03、INT-T16～INT-T17 |
