...
- 通知、作业、独立队伍和问卷是四个一级业务域；“校内赛”只保留为独立队伍中心的导航名称。提交版本属于作业上下文，队伍与问卷回答属于各自业务上下文，不作为全站公共内容。
...
└── /admin
    ...
    ├── /admin/competitions（独立队伍目录）
    │   ├── /admin/competitions/[teamId]
    │   └── /admin/competitions/new 与旧嵌套队伍路径（兼容重定向）
    ├── /admin/intentions
    ├── /admin/help
...
桌面使用左侧栏，依次包含概览、通知、作业、知识库同步、校内赛、问卷管理、反馈答疑、用户、方向设置、登录人员、邮件任务和审计日志。“反馈答疑”入口在当前管理员本人存在未读新工单提醒时显示蓝点，打开对应详情即只清除本人的该工单提醒；解决或删除工单后其余管理员的待处理提醒失效。通知与作业编辑页提供带二次确认的删除操作：未发布内容物理删除，已发布内容归档删除并从学生入口隐藏；问卷卡片和反馈答疑详情提供二次确认物理删除，问卷连同全部回答永久清除并从管理/学生界面退出，反馈答疑同步退出学生本人记录和匿名公开答疑。用户页为非当前账号提供独立永久删除确认，要求内部原因、管理员当前密码、目标邮箱和近期 PostgreSQL/MinIO 备份可恢复确认；管理员注销本人账号必须走个人资料页。“校内赛”入口只展示独立队伍目录，不提供赛事创建、公告、报名、阶段、赛题或作品操作；队伍详情提供带内部原因的补录、移除、队长转让和二次确认物理删除。管理员真实角色保持 `admin`，可通过侧栏底部“查看学生视图”切换当前 Session 的有效角色；学生视图隐藏管理入口并显示“培训文档”和学生反馈答疑入口，但后端仍独立鉴权，真实角色不会改变。
...
flowchart TD
    ...
    Dashboard --> TeamCenter[校内赛：独立队伍中心]
...
`本人个人资料页或管理员用户页发起 → 重新输入当前密码 → 精确确认目标邮箱 → 管理员额外填写原因并确认近期双存储备份 → 服务端再次校验最后管理员和锁定数据 → PostgreSQL 事务删除个人数据并写审计/对象清理 Outbox → 成功后本人回到登录页或管理员列表移除目标账号 → Worker 幂等清理个人 MinIO 对象`。数据库提交失败时账号和对象授权均保持原状；Worker 暂时失败时对象随机键不可再授权，任务按退避继续清理。
...
`管理员创建多题问卷并设置提交上限与全部学生/指定技术组范围 → 首次开放时复核目标组 → 开放并生成二维码或发布站内入口 → 当前技术组命中的学生登录 → 完成全部题目并提交 → 管理员按需修正文案/题目文字/时间/上限/范围且保持题量、题型、选项冻结 → 未达上限时再次提交最新答案 → 管理员查看目标学生统计和实名名单 → 关闭、按当前配置重新开启或归档 → 必要时二次确认永久删除并从全部问卷入口移除`。

### 管理员按第一志愿配置方向

`管理员读取问卷第一道单选题 → 为全部选项选择启用方向 → 阅读覆盖和历史受众提示 → 二次确认 → 服务端以当前最新回答锁定并更新激活学生方向 → 展示符合条件、更新、未变化和跳过数量`。该流程是显式快照应用，不是持续同步。

### 反馈答疑

`学生选择系统反馈或问题答疑 → 提交标题与详情并同事务提醒全部当前有效管理员 → 管理员“反馈答疑”入口出现本人蓝点 → 管理员打开详情后只标记本人提醒已读 → 填写处理结果/答复 → 工单变为已解决、其余管理员待处理提醒失效并发送学生站内提醒 → 学生在本人详情查看答复；若类型为问题答疑，则同时进入登录态匿名公开答疑供其他用户阅读；管理员必要时二次确认物理删除，所有读取立即不可见且未读提醒失效`。该流程不发送管理员邮件，也不把标题、正文或学生身份写入管理员提醒。

### 优秀作业标记

`管理员查看作业提交版本 → 标记为优秀作业 → 对应作业详情显示 → 作业受众查看 → 必要时取消标记`。队伍不拥有提交，也不进入该流程。
...
- 学生个人版本详情的上一级固定为 `/assignments/{assignment_id}` 原作业详情；管理员提交审阅详情的上一级固定为 `/admin/assignments/{assignment_id}/edit` 原作业管理页，不返回工作台或全局提交入口。
...
| 管理后台 | AUTH-007～AUTH-008、NEWS-001～NEWS-009、HW-001～HW-008、TEAM-008、INT-001～INT-010、HELP-003～HELP-005、HELP-008、MAIL-002～MAIL-005、NFR-006 |
