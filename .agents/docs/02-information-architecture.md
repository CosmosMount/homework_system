...
`学生选择系统反馈或问题答疑 → 提交标题与详情 → 在本人列表跟踪待处理状态 → 管理员查看并填写处理结果/答复 → 工单变为已解决并发送站内提醒 → 学生在本人详情查看答复；若类型为问题答疑，则同时进入登录态匿名公开答疑供其他用户阅读；管理员必要时二次确认物理删除，所有读取立即不可见且未读提醒失效`。
