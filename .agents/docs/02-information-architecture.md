...
- 管理员用户列表在服务端对全体账号执行姓名、邮箱、学号、角色和状态搜索，并使用 URL 分页逐页查看所有账号；不得只加载固定前 100 条后在浏览器内过滤。越界页回到实际末页，用户写操作成功后按同一 URL 查询刷新并在末页收缩时回退。
...
| 管理后台 | AUTH-007～AUTH-008、NEWS-001～NEWS-009、HW-001～HW-008、COMP-001～COMP-006、INT-001～INT-006、HELP-003～HELP-005、HELP-008、MAIL-002～MAIL-005、NFR-006 |
