...
- 单个正式提交版本最多关联 100 个不重复附件，附件总量上限为 2 GiB，即 2147483648 字节。
...
作业页面允许一次选择多张图片或其他任务白名单内附件，并先按当前版本剩余数量和剩余字节整体预检；通知页面继续单文件选择。批量只是客户端编排：浏览器按选择顺序逐文件执行以下会话，避免同时计算多个大文件哈希或扩大上传资源峰值，任一失败时保留当前文件和后续队列供恢复。客户端为每个文件分别提交原文件名、字节数、声明媒体类型、完整 SHA-256、目的和上下文。服务端：
...
- 完整/增量对象备份只接受服务端 `objects/` 与 `knowledge/` 前缀并保留原对象键；路径穿越或其他前缀必须硬失败，不能直接映射到宿主文件系统。
- 存储对账同时把普通 `files` 与 `knowledge_assets` 作为数据库引用；引用缺失、大小或 SHA-256 不符阻断恢复。历史孤立知识库对象只报告计数并保留，未经独立审计不得自动删除，普通日志不得输出对象键。
...
- 账号擦除按业务引用区分对象：目标账号未绑定的上传和个人作业提交附件属于个人对象，随 PostgreSQL 账号事务删除元数据并创建 `delete_account_object` Outbox；已绑定通知或团队赛事版本的附件属于共享资源，继续保留并把 `owner_user_id` 置空。
- 账号对象清理任务与用户行、个人引用和脱敏审计在同一数据库事务提交，避免“账号已删但没有对象清理凭据”；任务只使用服务端生成的随机对象键，multipart 标识由 Outbox 独立密钥认证加密。
- Worker 先幂等执行 `abort_multipart`，再执行对象删除；multipart 或对象已经不存在视为成功。对象存储异常按既有退避重试，成功后清空任务载荷，最终失败摘要和运维日志不得包含对象键或上传标识。
- Worker 暂停不回滚已经提交的账号删除，数据库引用消失后对象不能通过业务 API 获得签名；恢复 Worker 后继续处理积压。禁止依据桶扫描或已删除上传者身份猜测清理范围。
...
| 账号删除后 Worker 暂停或 MinIO 暂时失败 | PostgreSQL 删除保持提交且对象不再授权；可靠 Outbox 在 Worker 恢复后先终止 multipart 再重试删除 |
...
- 账号删除时个人对象与通知/团队共享附件分类正确；重复终止 multipart/删除幂等，对象存储故障只更新脱敏重试状态，备份副本不被在线清理任务改写。
- 优秀作业标记期间的删除保护、取消标记后的合规删除和 Worker 重复清理。

## 知识库媒体本地化

- Worker 或首次上线内部命令下载飞书图片、白板、正文附件、正文 Drive 文件引用和目录 `obj_type=file` 独立文件时先执行响应大小上限；图片只接受 PNG、JPEG、GIF、WebP 魔数，普通文件复用全局安全扩展名、危险后缀、媒体类型和内容嗅探策略。知识库专用文件入口额外接受单一末扩展名 `.exe`，但安全/危险双扩展仍拒绝；首个 64 KiB 必须包含有效 DOS/PE 结构、支持的 x86/x64/ARM/ARM64 架构、PE32/PE32+ optional header、合理 section 数和可执行映像标志，DLL 标志必须为空。`FEISHU_KNOWLEDGE_MAX_FILE_BYTES` 默认 1 GiB，约束块附件、富文本内嵌附件、正文 Drive 文件和目录独立文件；`FEISHU_KNOWLEDGE_MAX_ASSET_BYTES` 默认 50 MiB，只约束图片和白板，不放宽视觉媒体。
- 飞书文件按流读取：打开时检查可用的 `Content-Length`，读取中累计字节并在第一个超限字节立即关闭；Service 最多保留 64 KiB 嗅探首块，文件名、危险后缀、扩展名、魔数和媒体类型通过后才创建 MinIO multipart。对象存储以固定 16 MiB 分片上传，每片携带 SHA-256 校验和并按 ETag 顺序完成，同时增量计算完整大小、SHA-256 和首 32 字节；上游、校验、MinIO 或取消异常都必须关闭响应并尽力 abort，abort 失败不得覆盖原始业务异常。不得生成宿主明文临时文件或把文件内容写入 PostgreSQL。
- 对齐参考提交 `c28f8a0` 后，图片与附件共用请求前等待 350 ms 的串行队列；白板独立请求并发送 `Accept: image/png`。已存在媒体按外部 token 复用，但对象键仍由本平台生成。
- SVG、HTML、脚本、未经上述 PE 校验的可执行文件、双危险后缀、超限、空内容或下载失败媒体不进入 MinIO。正文图片、块附件和内嵌附件从飞书 Drive `medias` 端点读取；受信 `/file/{token}` 正文引用与 Wiki 目录独立文件从 Drive `files` 端点读取。两类结果进入同一扩展名、内容探测、大小和对象键校验链路。通过校验的 EXE 元数据使用 `application/vnd.microsoft.portable-executable`，但签名下载覆盖为 `Content-Disposition: attachment` 和 `Content-Type: application/octet-stream`。图片或白板失败跳过对应块；正文文件失败保留 `type_not_allowed/too_large/unavailable` 与真实管理员回退元数据，目录独立文件失败保留 `asset_id=NULL` 的不可下载节点。真实学生和管理员学生视图不得把失败资源渲染为飞书链接，真实管理员普通视图只保留正文来源回退用于排障。
- 对象键使用 `knowledge/<asset_uuid>/<server_uuid>.<ext>`，全部 UUID 由服务端生成；PostgreSQL 只保存元数据、SHA-256 和引用。
- 图片/白板授权后生成 5 分钟 inline 签名，正文附件、富文本内嵌文件、Drive 文件引用和目录独立文件生成 5 分钟 attachment 签名。授权必须命中文档资源关联或最新成功运行的独立文件节点关联；直接猜测资源 UUID、旧快照资源或对象键均不能通过业务授权。
