...
- Worker 或首次上线内部命令下载飞书图片、白板、正文附件和目录 `obj_type=file` 独立文件时先执行响应大小上限；图片只接受 PNG、JPEG、GIF、WebP 魔数，正文附件与目录文件复用全局安全扩展名、危险后缀、媒体类型和内容嗅探策略。
- 对齐参考提交 `c28f8a0` 后，图片与附件共用请求前等待 350 ms 的串行队列；白板独立请求并发送 `Accept: image/png`。已存在媒体按外部 token 复用，但对象键仍由本平台生成。
- SVG、HTML、脚本、可执行文件、双危险后缀、超限、空内容或下载失败媒体不进入 MinIO。正文图片/附件从飞书 Drive `medias` 端点读取，Wiki 目录独立文件从 Drive `files` 端点读取；两类结果进入同一扩展名、内容探测、大小和对象键校验链路。图片或白板失败跳过对应块；正文附件失败保留整篇飞书原文回退元数据，目录独立文件失败保留 `asset_id=NULL` 的不可下载节点。真实学生和管理员学生视图不得把两类失败资源渲染为飞书链接，真实管理员普通视图只保留正文来源回退用于排障。
- 对象键使用 `knowledge/<asset_uuid>/<server_uuid>.<ext>`，全部 UUID 由服务端生成；PostgreSQL 只保存元数据、SHA-256 和引用。
- 图片/白板授权后生成 5 分钟 inline 签名，正文附件、富文本内嵌文件和目录独立文件生成 5 分钟 attachment 签名。授权必须命中文档资源关联或最新成功运行的独立文件节点关联；直接猜测资源 UUID、旧快照资源或对象键均不能通过业务授权。
