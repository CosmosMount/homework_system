...
- 真实管理员可在个人提交审阅页确认当前最新正式版本完成或撤销确认；操作由后端管理员依赖、CSRF、提交行锁和脱敏审计保护，重复确认/撤销幂等，不发送提醒或邮件。
- 确认绑定不可变版本。学生提交新版本后保留旧确认版本和时间，但 `is_completed=false` 并回到待确认；管理员作业详情将当前受众互斥分为已提交、已完成、未提交三栏，历史提交者继续单列，列表与统计显示完成进度。
- 新迁移 `20260908_0023` 为 `submissions` 增加完成版本、时间和操作者，使用同提交复合外键、管理员 `SET NULL` 外键及状态一致性检查；账号擦除预处理同步清空新增引用。隔离恢复副本完成 `0022 → 0023 → 0022 → 0023` 往返并通过 `alembic check`，生产仅前滚一次到 `0023 (head)`。
- 后端完整 422 项 Pytest、Ruff、180 文件格式和 155 文件严格 Mypy 通过；前端完整 26 文件/156 项 Vitest、ESLint、严格 TypeScript 与 Next.js 16.3.2 生产构建通过。固定标签 `assignment-completion-20260908` 已两阶段上线，Backend/Worker 为 `sha256:41e4bf16bb57…`，Frontend 为 `sha256:80dc5b64e94e…`。
- 发布前 OpenPGP 每日增量备份 `pnx-backup-20260908T045305Z-daily` 已从全新空卷恢复 1,384 个对象并零差异对账；生产六服务 healthy、重启 0，运行 OpenAPI 为 108 条路径且完成确认端点匿名 POST/DELETE 均为 401。发布前后用户、作业、受众、提交、版本、文件和 Outbox 聚合不变，PostgreSQL/MinIO 容器与数据卷未重建。


### 生产对象与冗余备份收敛

- 删除 1,875 个超过 7 天且无普通文件/知识库资产数据库引用的历史 `knowledge/` 对象，释放 1,336,875,776 字节；清理后 1,384 个 MinIO 对象与有效引用完全一致，缺失、大小、SHA-256 和未引用差异均为 0。
- 新加密每日增量备份 `pnx-backup-20260908T032301Z-daily` 已从全新空卷恢复 1,384 个对象，RPO 61 秒、RTO 161 秒。备份目录只保留唯一周基线与该最新每日增量，三套旧每日备份和三组测试伪备份已删除。
- 生产六服务继续 healthy，PostgreSQL/MinIO 容器和数据卷未重建；没有数据库行或业务写入。
