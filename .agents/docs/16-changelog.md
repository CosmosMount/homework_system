...
- 新迁移 `20260908_0023` 为 `submissions` 增加完成版本、时间和操作者，使用同提交复合外键、管理员 `SET NULL` 外键及状态一致性检查；账号擦除预处理同步清空新增引用。离线升降级 SQL 生成通过，未连接生产数据库。
- 后端完整 422 项 Pytest、Ruff、180 文件格式和 155 文件严格 Mypy通过；前端完整 26 文件/156 项 Vitest、ESLint、严格 TypeScript 与 Next.js 16.3.2 生产构建通过。候选未部署，生产仍为 `20260908_0022 (head)`。
