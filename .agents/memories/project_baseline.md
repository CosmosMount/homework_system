...
2026-09-06 当前生产统一版本为 `unified-assignment-teams-20260906`：Backend/Worker `sha256:9c2701a53ab1…`、Frontend `sha256:2b908afcfcae…`，六服务 healthy、重启 0，Alembic 为 `20260904_0020 (head)` 且无模型漂移。已发布/已关闭作业可修改受众，管理端完整区分当前受众已提交、未提交和退出受众历史提交者；作业可批量上传多张图片，评语修订可靠创建不含正文的邮件任务。旧赛事队伍已隔离到 legacy 表，当前独立队伍从空表开始，运行 OpenAPI 为 98 条路径且不暴露赛事 API。唯一保留的生产恢复材料为 `pnx-backup-20260905T163853Z-weekly` 三件套，已完成空卷原样恢复、3,291 对象零差异对账和统一镜像升级复核；旧应用镜像均已删除。
