...
当前备份链已收敛为 `pnx-backup-20260905T163853Z-weekly` 周基线与 `pnx-backup-20260908T032301Z-daily` 最新每日增量。后者已从空 PostgreSQL/MinIO 卷恢复 1,384 个对象，缺失、大小、SHA-256 与未引用差异均为 0，RPO 61 秒、RTO 161 秒。生产已删除 1,875 个超过 7 天且无数据库引用的历史知识库对象，释放 1,336,875,776 字节；生产 PostgreSQL/MinIO 容器及数据卷未重建。
...
- 新迁移 `20260908_0023` 为 `submissions` 增加完成版本、时间和操作者，包含同提交复合外键、管理员 `SET NULL` 外键和状态一致性检查；账号擦除预处理同步清空完成引用。离线升降级 SQL 可生成，运行生产仍为 `20260908_0022`。
- 后端完整 422 项 Pytest、Ruff、180 文件格式和 155 文件严格 Mypy，前端 26 文件/156 项 Vitest、ESLint、严格 TypeScript 与 Next.js 16.3.2 构建通过。未连接或修改生产 PostgreSQL/MinIO，未构建镜像或部署。
