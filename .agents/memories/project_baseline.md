...
2026-09-08 当前生产版本为 `assignment-completion-20260908`：Backend/Worker `sha256:41e4bf16bb57…`、Frontend `sha256:80dc5b64e94e…`，六服务 healthy、重启 0，Alembic 为 `20260908_0023 (head)` 且无模型漂移。管理员可确认个人提交当前最新版本完成或撤销；作业总览按当前受众显示已提交、已完成、未提交三栏。运行 OpenAPI 为 108 条路径。

当前备份链为 `pnx-backup-20260905T163853Z-weekly` 周基线与 `pnx-backup-20260908T045305Z-daily` 最新每日增量。后者已从空 PostgreSQL/MinIO 卷恢复 1,384 个对象，缺失、大小、SHA-256 与未引用差异均为 0；恢复副本完成 `0022 → 0023 → 0022 → 0023` 与模型漂移检查。生产 PostgreSQL/MinIO 容器及数据卷未重建。
...
## 2026-09-08 作业完成确认与三栏总览部署基线

- ADR-065 已接受：完成确认是管理员针对个人提交当前最新不可变版本形成的业务事实；`completed_version_id = latest_version_id` 才派生已完成。学生新版本保留旧确认但回到待确认，确认/撤销幂等且只写脱敏审计，不发送提醒或邮件。
- 管理员作业详情把当前受众互斥分为已提交待确认、已完成、未提交三栏，历史提交者继续单列；个人审阅页可确认/撤销并展示旧确认失效提示，作业列表和统计展示完成进度。
- 新迁移 `20260908_0023` 为 `submissions` 增加完成版本、时间和操作者，包含同提交复合外键、管理员 `SET NULL` 外键和状态一致性检查；账号擦除预处理同步清空完成引用。隔离副本完成升降级往返，生产仅前滚一次且无模型漂移。
- 后端完整 422 项 Pytest、Ruff、180 文件格式和 155 文件严格 Mypy，前端 26 文件/156 项 Vitest、ESLint、严格 TypeScript 与 Next.js 16.3.2 构建通过。固定标签已两阶段上线；六服务 healthy、重启 0，匿名权限、运行产物、业务聚合、对象对账和稳定期日志全部通过。
